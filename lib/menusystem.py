...
BUTTON = 1
SWITCH = 0
FONT = font.Font(join(dirname(__file__), "../data/Roboto-Regular.ttf"), 20)
try:
    Arrow = "»".decode('utf-8')
...
def init():
    ...
    if not DISPLAY:
        raise AttributeError('set video before init MenuSystem')
            ...
            if self.itemsrect.clip(self).collidepoint(ev.pos):
                ...
                if ev.button == 5:
                    if self.itemsrect.bottom > self.bottom:
                        self.itemsrect.top -= self.lineheight
                        if self.itemsrect.bottom < self.bottom:
                            self.itemsrect.bottom = self.bottom
                        self.index = (y - self.itemsrect.top) // self.lineheight
                        return True
            ...
            if not self or self.boxindex == None:
                if ev.type == MOUSEBUTTONUP:
                    if self.index > -1 and ev.button == 1:
                        if not ret:
                            ret = super(MenuBar, self).set(
                                self.menuboxlist[self.index], self.rects[self.index].bottomleft)
                    elif ret:
                        ret += [self.draw()]
...
class MenuChoice(MenuSystem, object):
    ...
    def draw(self):
        ...
        gfxdraw.hline(DISPLAY, self.rect.left + 1, self.rect.right - 1, self.rect.bottom - 1, BORDER_RIGHT)
        x = self.rect.x + self.lineheigth / 3
        if self.mouse_in or self:
            gfxdraw.box(DISPLAY, self.rect.inflate(-2, -2), BGHIGHTLIGHT)
        clipxy = DISPLAY.get_clip()
